...
func TestRoute_NeedsNewVolume(t *testing.T) {
	...
	got := Route(s)
	if got == nil || got.Agent != "architect_long" || got.Reason != "Cuối tập cần quyết định thêm tập mới, tập kết hay kết thúc toàn bộ tác phẩm" {
...
func TestRoute_ExternalRevisionDispatchesArchitectBeforeWriter(t *testing.T) {
	...
	if got == nil || got.Agent != "architect_short" || !strings.Contains(got.Reason, "2 phản hồi") {
...
func TestRoute_PlanningFillDispatchesSamePlanner(t *testing.T) {
	...
	for _, want := range []string{"Bổ sung các mục thiết lập nền còn thiếu", "characters", "world_rules", "save_foundation"} {
