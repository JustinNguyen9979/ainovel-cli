...
func formatExportSuccess(res *exp.Result, languages ...utils.Language) string {
	lang := resolveLanguage(languages)
	var b strings.Builder
	fmt.Fprintf(&b, ui(lang, "✓ 已导出 %d 章 / %s 到 %s", "✓ Đã xuất %d chương / %s vào %s"), res.Chapters, humanBytes(res.Bytes), res.Path)
	if n := len(res.Skipped); n > 0 {
		fmt.Fprintf(&b, ui(lang, "（跳过 %d 章未完成：%s）", " (bỏ qua %d chương chưa hoàn thành: %s)"), n, briefIntList(res.Skipped, 5))
	}
	return b.String()
}
