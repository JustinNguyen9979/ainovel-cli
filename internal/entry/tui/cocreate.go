...
func placeholderForNewMode(mode startupMode) string {
	return localizedModePlaceholder(mode, utils.LanguageVI)
}

func placeholderForCoCreate(state *cocreateState) string {
	return localizedCoCreatePlaceholder(state, utils.LanguageVI)
}
...
func newCoCreateState(initial string, languages ...utils.Language) *cocreateState {
...
func stageCoCreateOpener(lang utils.Language) string {
	return ui(lang, "我先暂停一下，想和你一起规划接下来的走向。", "Tôi muốn tạm dừng một chút để cùng bạn lập kế hoạch cho hướng đi tiếp theo.")
}
...
func newStageCoCreateState(languages ...utils.Language) *cocreateState {
	lang := resolveLanguage(languages)
	s := newCoCreateState(stageCoCreateOpener(lang), lang)
	s.stage = true
	return s
}
...
func renderStartupModeBar(width int, mode startupMode, languages ...utils.Language) string {
	lang := resolveLanguage(languages)
	quickLabel, _, _ := localizedNewMode(startupModeQuick, lang)
	cocreateLabel, _, _ := localizedNewMode(startupModeCoCreate, lang)
	quick := renderStartupModePill(mode == startupModeQuick, quickLabel)
	cocreate := renderStartupModePill(mode == startupModeCoCreate, cocreateLabel)
	title := lipgloss.NewStyle().
		Foreground(colorAccent).
		Bold(true).
		Render(tr(languages, utils.MsgStartupMode))
...
func renderCoCreateBody(width, height int, state *cocreateState, errMsg, inputView string, spinnerFrame int, languages ...utils.Language) string {
	if state == nil {
		return ""
	}
	...
	suggestionsBox := renderCoCreateSuggestions(innerW, state, languages...)
	suggestionsH := 0
	if suggestionsBox != "" {
	...
	convPanel := renderCoCreateConversationPanel(innerW, convH, state, errMsg, spinnerFrame, languages...)
	...
	rightPanel := renderCoCreatePromptPanel(rightW, height, state, languages...)
...
func renderCoCreateSuggestions(width int, state *cocreateState, languages ...utils.Language) string {
	...
	lines := []string{hintStyle.Render(ui(resolveLanguage(languages), "AI 建议（按 1/2/3 组合，可编辑后发送）：", "Gợi ý của AI (nhấn 1/2/3 để ghép, có thể sửa trước khi gửi):"))}
	for i, s := range sugs {
...
func renderCoCreateModal(width, height int, state *cocreateState, errMsg, inputView string, spinnerFrame int, quitPending bool, languages ...utils.Language) string {
	if state == nil {
		return ""
	}
	...
	lang := resolveLanguage(languages)
	titleText, subtitleText, _ := localizedNewMode(startupModeCoCreate, lang)
	if state.stage {
		titleText = ui(lang, "阶段共创", "Cùng lập kế hoạch giai đoạn")
		subtitleText = ui(lang, "规划后续走向，再继续创作", "Lập kế hoạch hướng tiếp theo rồi tiếp tục sáng tác")
	}
	...
	if quitPending {
		...
		hintLine = hintStyle.Foreground(lipgloss.Color("243")).Bold(true).Render(localizedCompletionHint(lang))
	} else {
		hintLine = hintStyle.Foreground(colorDim).Italic(true).Render(coCreateHint(state, lang))
	}

	body := renderCoCreateBody(boxW-4, contentH, state, errMsg, inputView, spinnerFrame, languages...)
...
func coCreateHint(state *cocreateState, languages ...utils.Language) string {
	lang := resolveLanguage(languages)
	switch {
	case state == nil:
		return ui(lang, "Enter 发送 · Esc 退出", "Enter gửi · Esc thoát")
	case state.awaiting:
		return ui(lang, "AI 回复中 · ↑↓ 滚对话 · 滚轮滚指令 · Esc 退出", "AI đang trả lời · ↑↓ cuộn hội thoại · cuộn chỉ dẫn · Esc thoát")
	case state.canStart():
		action := ui(lang, "Ctrl+S 开始创作", "Ctrl+S bắt đầu sáng tác")
		if state.stage {
			action = ui(lang, "Ctrl+S 应用并继续", "Ctrl+S áp dụng và tiếp tục")
		}
		return ui(lang, "Enter 继续补充 · ", "Enter tiếp tục bổ sung · ") + action + ui(lang, " · ↑↓ 滚对话 · 滚轮滚指令 · Esc 退出", " · ↑↓ cuộn hội thoại · cuộn chỉ dẫn · Esc thoát")
	default:
		return ui(lang, "Enter 发送 · ↑↓ 滚对话 · 滚轮滚指令 · Esc 退出", "Enter gửi · ↑↓ cuộn hội thoại · cuộn chỉ dẫn · Esc thoát")
	}
}

func renderCoCreateConversationPanel(width, height int, state *cocreateState, errMsg string, spinnerFrame int, languages ...utils.Language) string {
	lang := resolveLanguage(languages)
	...
	userRole := lipgloss.NewStyle().Foreground(colorAccent2).Bold(true).Render(ui(lang, "你", "Bạn"))
	...
	thinkingTag := lipgloss.NewStyle().Foreground(colorDim).Bold(true).Render(ui(lang, "AI 思考", "AI đang suy nghĩ"))
	...
	for i, item := range state.session.History() {
		...
		if isUser && state.stage && i == 0 {
			systemLine := ui(lang, stageCoCreateSystemLine, "Đã tạm dừng sáng tác, chuyển sang cùng lập kế hoạch — AI sẽ dựa trên tiến độ hiện tại để cùng bạn lập kế hoạch hướng tiếp theo.")
	...
	style := lipgloss.NewStyle().
		...
		Height(height).
		Padding(0, 1)
	return style.Render(panelTitleStyle.Render(":: "+ui(lang, "共创对话", "Trao đổi cùng AI")) + "\n" + state.convVP.View())
}

func renderCoCreatePromptPanel(width, height int, state *cocreateState, languages ...utils.Language) string {
	lang := resolveLanguage(languages)
	readyLabel := ui(lang, "已可开始创作", "Đã có thể bắt đầu sáng tác")
	if state.stage {
		readyLabel = ui(lang, "已可应用并继续", "Đã có thể áp dụng và tiếp tục")
	}
	status := lipgloss.NewStyle().Foreground(colorDim).Render(ui(lang, "继续对话中", "Đang tiếp tục trao đổi"))
	if state.ready() {
		status = lipgloss.NewStyle().Foreground(colorAccent).Render(readyLabel)
	}
	if state.awaiting {
		status = lipgloss.NewStyle().Foreground(colorMuted).Italic(true).Render(ui(lang, "AI 整理中", "AI đang整理"))
	}
	...
	emptyHint := ui(lang, "AI 会在这里持续整理出一段可直接进入创作的最终指令。", "AI sẽ liên tục sắp xếp chỉ dẫn cuối cùng để bắt đầu sáng tác tại đây.")
	panelTitle := ":: " + ui(lang, "当前创作指令", "Chỉ dẫn sáng tác hiện tại")
	if state.stage {
		emptyHint = ui(lang, "AI 会在这里持续整理出后续阶段的方向 brief。", "AI sẽ liên tục sắp xếp brief hướng đi cho giai đoạn tiếp theo tại đây.")
		panelTitle = ":: " + ui(lang, "后续方向", "Hướng đi tiếp theo")
	}
	...
	if state.promptVP.TotalLineCount() > state.promptVP.VisibleLineCount() {
		...
		case state.promptVP.AtTop():
			hint = ui(lang, "↓ 下方还有内容，可滚轮或 PgDn 查看", "↓ Còn nội dung bên dưới, dùng con lăn hoặc PgDn để xem")
		case state.promptVP.AtBottom():
			hint = ui(lang, "↑ 上方还有内容，可滚轮或 PgUp 查看", "↑ Còn nội dung bên trên, dùng con lăn hoặc PgUp để xem")
		default:
			hint = ui(lang, "↑↓ 可继续滚动查看", "↑↓ Tiếp tục cuộn để xem")
		}
	}
